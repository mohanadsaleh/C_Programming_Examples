...
//*Aranacak öğrenci no girilecek ve uyan kayıt varsa tüm bilgisi listelenece
    if (bas == NULL) {
        printf("Kayit bulunamadi.\n");
        return;
    }
...
void listLess(struct ogrenci* bas) {
	// listede başarı notu 60’ın altında olan öğrencileri listeden silecektir.
    struct ogrenci* temp = bas;
    if (temp == NULL)
        printf("Kayit Yok\n");
...
void updateAhmet(struct ogrenci* bas) {
	//“Ahmet” olan tüm kayıtları “AHMET” olarak güncelleyecek.
    struct ogrenci* temp = bas;
    if (temp == NULL) {
    	printf("------------------------------------------\n");
        printf("KAYIT YOK\n");
...
void exchange(struct ogrenci* bas) {
	...
	//e listede yer alan ilk kayıt ile son kaydı yer değiştirecektir Fonksiyonu
